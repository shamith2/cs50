...
{
    // TODO
    // record preference for candidates
    for (int idx = 0; idx < candidate_count; ++idx)
    {
        if (strcmp(candidates[idx].name, name) == 0)
        {
            // preferences[i][j] is jth preference for voter i
            // idx is the candidate index
...
void tabulate(void)
{
    ...
    for (int i = 0; i < voter_count; ++i)
    {
        ...
        // candidate index = preferences[i][rank]

        // while candidate is eliminated,
        // go to next rank
...
{
    // TODO
    int max_val = voter_count / 2;   // max vote value
...
int find_min(void)
{
    // TODO
    // assumed min value
    // min cannot be more than voter_count
    int min = voter_count;

    // linear search to find min value
    for (int i = 0; i < candidate_count && candidates[i].eliminated == false; ++i)
    {
        int cand_votes = candidates[i].votes;
        if (cand_votes <= min)
        {
            min = cand_votes;
        }
